...
class Auto(BotPlugin):
   ...
   def activate(self):
	...
	try:
		Popen(['/usr/bin/automater/Automater.py -b -r'], stdout=PIPE, stderr=STDOUT).communicate()[0]
	except Exception as e:
		self.warn_admins("This plugin uses automater and it wasn't found on your system")

   @botcmd
   def Auto(self, mess, args):
      return Popen(['/usr/bin/automater/Automater.py -b -r'] + args.split(), stdout=PIPE, stderr=STDOUT).communicate()[0].decode('utf-8')
